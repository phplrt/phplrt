...
enum RegexModifier: string
{
     ...
     * as well as at the very start and end. This is an equivalent to Perl's
     ...
     * more time analyzing it to speed up the time taken for matching.
     ...
     * invalid pattern will trigger an error of level {@see E_WARNING}.
     */
    case Utf8 = 'u';

    /**
     * This modifier makes simple (`xyz`) groups non-capturing. Only named
     * groups like `(?<name>xyz)` are captured. This only affects which groups
     * are capturing, it is still possible to use numbered subpattern references,
     * and the matches array will still contain numbered results.
     */
    case NoAutoCapture = 'n';

    /**
     * When {@see RegexModifier::Utf8} and {@see RegexModifier::Caseless} are in effect, this
     * modifier prevents matching across ASCII and non-ASCII characters.
     *
     * For example,
     * - `preg_match('/\x{212A}/iu', "K")` - When {@see RegexModifier::ExtraCaselessRestrict}
     *   is used, then it matches the Kelvin sign K (`U+212A`).
     *
     * - `preg_match('/\x{212A}/iur', "K")` - Otherwise, it does not match.
     */
    case ExtraCaselessRestrict = 'r';
}
