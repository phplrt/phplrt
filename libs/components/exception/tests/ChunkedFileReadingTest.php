...
final class ChunkedFileReadingTest extends TestCase
{
    ...
    public function testReadsFileByChunks(): void
    {
        ...
        try {
            ...
            Assert::same(self::describe(self::read($reader, $source, new FailureInterval(23, 15), 2)), [
                ' #2@7: line 2',
                ' #3@14: line 3',
                '>#4@21:2-6: line 4',
                '>#5@28:0-6: line 5',
                ...
                for ($offset = -1, $size = \strlen($code); $offset <= $size + 1; ++$offset) {
                    foreach ([0, 1, 3] as $lines) {
                        foreach ([0, 5, 21] as $length) {
                            ...
                            Assert::same(self::describe(self::read($reader, new FileSource($pathname), $fragment, $lines)), self::describe(self::read($reader, new StringSource($code), $fragment, $lines)), \sprintf(
